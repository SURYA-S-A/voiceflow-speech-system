...
import { formatDateForChatWindow, getCurrentDatetime } from "@/utils/dateUtils";
import { getWsUrl } from "@/utils/urlUtils";
import { useEffect, useRef, useState } from "react";

export default function UpdatedVoicePage() {
    ...
    const addMessage = (type: 'user' | 'bot', text: string) => {
        ...
        console.log(`📝 Adding ${botMessage.sender} message: ${botMessage.text}`);
    ...
    const getStatusColor = () => {
        if (isBotTyping) return 'bg-orange-500';  // ✅ Use state for UI
                                ...
                                >
                                    <div className="flex items-start justify-between">
                                        <div className="flex-1">
                                            <span className={`text-sm font-semibold ${msg.sender === 'user' ? 'text-blue-600 dark:text-blue-400' :
                                                msg.sender === 'bot' ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'
                                                }`}>
                                                {msg.sender === 'user' ? '👤 You' :
                                                    msg.sender === 'bot' ? '🤖 Assistant' : '❌ Error'}:
                                            </span>
                                            <p className="text-gray-800 dark:text-gray-200 mt-1">{msg.text}</p>
                                        </div>
                                        <span className="text-xs text-gray-400 ml-2">
                                            {formatDateForChatWindow(msg.timestamp)}
                                        </span>
                                    </div>
                                </div>
                            ))
